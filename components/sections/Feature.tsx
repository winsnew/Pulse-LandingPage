...
const Features = () => {
    return (
        <section
            ...
            id="features"
        >
            {/* 🔲 Futuristic scanning grid lines */}
            <div className="absolute inset-0 z-20 overflow-hidden pointer-events-none">
                {Array.from({ length: 14 }).map((_, i) => (
                    <div
                        key={`v-${i}`}
                        className="absolute top-0 h-full w-[1px] bg-purple-500/20 blur-[0.5px] mix-blend-screen"
                        style={{
                            left: `${(i + 1) * 6}%`,
                ...
                {Array.from({ length: 10 }).map((_, i) => (
                    <div
                        key={`h-${i}`}
                        className="absolute left-0 w-full h-[1px] bg-purple-500/20 blur-[0.5px] mix-blend-screen"
                        style={{
                            top: `${(i + 1) * 8}%`,
