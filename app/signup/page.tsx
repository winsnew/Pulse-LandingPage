...
import { getPasswordErrors, isValidEmail, isValidPassword, validatePassword } from '@/lib/auth/utils';
...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const passwordValidCtx = validatePassword(formData.password);
    if (!isValidPassword(passwordValidCtx)) {
      const errors = getPasswordErrors(passwordValidCtx);
      setErrors(errors);
      return;
    }

    if (!agreeToTerms) {
      setErrors(['Please agree to the beta terms and privacy policy']);
      return;
    }
    ...
    try {
      ...
      // client side shouldn't know what exact internal caused errors.
      console.error('Registration error: something went wrong');
            ...
            {errors.length > 0 && (
              errors.length == 1 ? (
                <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                  <p className="text-red-300 text-sm text-center">{errors[0]}</p>
                </div>
              ) : (
                <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
